  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === "email") {
        ...
        payload.phone = form.identifier.startsWith("+")
          ? form.identifier
          : `+91${form.identifier.replace(/[^\d]/g, "")}`;
      }
      const { data } = await api.post("/auth/login", payload);
      const receivedToken =
        data.accessToken || data.token || data.jwt || data.idToken;
      if (!receivedToken) return setError("No token returned");
      console.log("[login] raw response keyss:", receivedToken);

      setToken(receivedToken);
      const adminFlag =
        data?.isAdmin === true || /admin/i.test(String(data?.role || ""));
      setIsAdmin(adminFlag);
      localStorage.setItem("isAdmin", adminFlag ? "true" : "false");
      if (adminFlag) {
        // Hard redirect to avoid any guard timing/race
      ...
      <form onSubmit={submit}>
        ...
        <div className="seg-toggle" style={{ marginBottom: "0.75rem" }}>
          <button
            ...
            className={`seg-option ${mode === "email" ? "active" : ""}`}
          >
            Email
          </button>
          <button
            ...
            onClick={() => {
              setMode("phone");
              setForm((f) => ({ ...f, identifier: "+91" }));
            }}
            className={`seg-option ${mode === "phone" ? "active" : ""}`}
          >
            Phone
          </button>
        </div>
        {mode === "email" ? (
          ...
          <PhoneInput
            value={form.identifier || "+91"}
            onChange={(val) => setForm({ ...form, identifier: val })}
          />
        )}
        <TextInput
          type="password"
