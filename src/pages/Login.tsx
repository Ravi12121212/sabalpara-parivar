...
const Login: React.FC = () => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate('/user-details'); // root will decide redirect based on hasProfile
    } catch (err: any) {
  ...
  return (
    <AuthCard title="Login" subtitle="" backTo="/signup">
      <form onSubmit={submit}>
        ...
        {mode === 'email' ? (
          <TextInput placeholder="Email" value={form.identifier} onChange={e => setForm({ ...form, identifier: e.target.value })} />
        ) : (
